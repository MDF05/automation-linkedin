...
Implementasi ``BaseAIProvider`` menggunakan Groq API (OpenAI-compatible).
...
# Groq pricing (USD per token) — llama-3.3-70b-versatile
_INPUT_COST_PER_TOKEN = 0.59 / 1_000_000   # $0.59 per 1M input tokens
_OUTPUT_COST_PER_TOKEN = 0.79 / 1_000_000  # $0.79 per 1M output tokens
...
class GroqProvider(BaseAIProvider):
    """
    Provider AI yang menggunakan Groq API.
    ...
    Konfigurasi:
      ...
      - Token limit: ``settings.ai_usage_limit_groq``
    ...
    async def generate(self, request: AIRequest) -> AIResponse:
        """
        Kirim request ke Groq API dan kembalikan AIResponse.
        ...
        try:
            ...
            logger.warning("GroqProvider: request timeout: %s", exc)
            return AIResponse(
                content="",
                provider=self.provider_name,
                prompt_tokens=0,
            ...
            latency_ms = int((time.monotonic() - t_start) * 1000)
            logger.warning("GroqProvider: unexpected error: %s", exc)
            return AIResponse(
                content="",
                provider=self.provider_name,
                prompt_tokens=0,
        ...
        try:
            ...
            logger.warning("GroqProvider: parse error: %s — data=%s", exc, data)
            return AIResponse(
                content="",
                provider=self.provider_name,
                prompt_tokens=0,
    ...
    def get_token_limit(self) -> int:
        ...
        Requirements: 8.3
        """
        return self._settings.ai_usage_limit_groq
